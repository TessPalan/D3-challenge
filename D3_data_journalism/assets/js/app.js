...
xText.append('text') // append a "text" element to the xText group
      ...
      .attr('class', 'axisText active x') // give it class of axisText, active, and x
...
xText.append('text') // append a "text" element to the xText group
      ...
      .attr('data-axis', 'x') // set the data-axis attribute to 'x'
      .attr('class', 'axisText inactive x')  // give it class of axisText, inactive, and x
...
yText.append('text') // append a "text" element to the yText group
      ...
      .attr('class','axisText active y') // give it class of axisText, active, and y
...
yText.append('text') // append a "text" element to the yText group
      ...
      .attr('data-axis', 'y') // set the data-axis attribute to 'y'
      .attr('class', 'axisText inactive y') // give it class of axisText, inactive, and y
...
function visualize(data) {
  ...
  circlesGroup.append('circle') // append a 'circle' element to the circlesGroup
            ...
            .on('mouseover', function(d) { // .on 'mouseover' event, fire off a function that takes argument d
              ...
              d3.select(this).style('stroke', 'DarkGreen'); // use d3.select() the this element, and modify the 'stroke' .style() to a color of your choosing
            })
            .on('mouseover', function(d) { // on 'mouseout' fire off a function that takes argument d
                // Remove the tooltip
                toolTip.hide(d, this); // use toolTip.hide() with d and this as the arguments
                // Remove highlight
                d3.select(this).style('stroke', 'MediumSeaGreen'); // use d3.select() the this element, and modify the 'stroke' .style() to another color of your choosing
            });
  ...
  circlesGroup.append('text') // append a 'text' element to circlesGroup
              ...
              .on('mouseover', function(d) { // on 'mouseover' event, fire off a function that takes argument d
                ...
                d3.select(this).style('stroke', 'DarkMagenta'); // use d3.select() the this element, and modify the 'stroke' .style() to a color of your choosing
              })
              .on('mouseout', function(d) { // on 'mouseout' fire off a function that takes argument d
                  // Remove the tooltip
                  toolTip.hide(d, this); // use toolTip.hide() with d and this as the arguments
                  // Remove highlight
                  d3.select(this).style('stroke', 'Plum'); // use d3.select() the this element, and modify the 'stroke' .style() to another color of your choosing
              });
    ...
    if (selectedLabel.classed('inactive', true)) { // if the selectedLabel has the class 'inactive'
      ...
      if (axis === 'x') { // if the axis is equal to 'x'
        ...
        // do all the same steps you just did for x, but this time do them for y
          ...
          yScale.domain([yMin, yMax]); // set the .domain() of xycale to be the [yMin, and yMax]
  
          // Now use a transition when we update the yAxis.
          svg.select('.yAxis')// select the .yAxis elements on the svg
              .transition() // set a transition()
              .duration(300) // give it a duration() of 300ms
              .call(yAxis); // call the yAxis
          ...
          d3.selectAll('circle').each(function() { // d3.selectAll() 'circle' elements, and then use .each to fire off an anonymous function with no arguments
            ...
            d3.select(this) // use d3.select(this)
              .transition() // set a transition
              .attr('cy', d => yScale(d[currentY])) // set the 'cy' attribute to go from d => xScale applied to d[currentY]
              .duration(300); // set the duration to 300ms
          });
          ...
          d3.selectAll('.stateText').each(function() { // d3.selectAll() '.stateText' elements, and then use .each to fire off an anonymous function with no arguments
            ...
            d3.select(this) // use d3.select(this)
              .transition() // set a transition
              .attr('dy', d => yScale(d[currentY])) // set the 'dy' attribute to go from d => xScale applied to d[currentY]
              .duration(300); // set the duration to 300ms
          });
  ...
  function resizeChart() { // define a function called resizeChart that takes no arguments
    ...
    svg.attr('width', width)
       .attr('height', height) // set the width and height attributes of the svg with the above variables
    ...
    d3.selectAll('circle') // d3.selectAll() 'circle' elements
      .attr('cy', yScale(d[currentY]))// set the 'cy' attribute to use the yScale() d[currentY]
      .attr('cx', xScale(d[currentX])) // set the 'cy' attribute to use the xScale() d[currentX]
